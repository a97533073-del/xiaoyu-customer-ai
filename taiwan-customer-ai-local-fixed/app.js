...
async function startVoice() {
  ...
  try {
    ...
    dc.onerror = () => { els.voiceHint.textContent = "語音資料通道發生錯誤。"; };
    ...
    const response = await fetch("./api/realtime", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sdp: offer.sdp, knowledge: state.knowledge })
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "建立語音連線失敗");
    ...
    addMessage("assistant", `語音目前無法連線：${error.message}`);
  } finally {
    els.voice.disabled = false;
  }
}
